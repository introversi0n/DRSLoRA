...
@dataclass
class MSPLoraConfig(PeftConfig):
    """
    ...
    def __post_init__(self):
        self.peft_type = PeftType.MSPLORA
...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, current_key=None, layer_num=None, middle_share_schedule=None, lora_linear_dict=None):
        if lora_linear_dict is None and layer_num is None and current_key is None and middle_share_schedule is None:
            ...
            match_adapter = re.search(r"(\d+)$", adapter_name)
            match_key = re.findall(r"\d+", current_key)
            if match_adapter and match_key:
                number_adapter = int(match_adapter.group(1))
                number_key = int(match_key[0])
                if number_adapter == 0:
                    if number_key == 0:
                        self.r[adapter_name] = r
                        self.lora_alpha[adapter_name] = lora_alpha
                        ...
                        share_module_name = re.sub(r"\d+", "0", current_key, count=1)
                        self.lora_dropout.update(nn.ModuleDict({adapter_name: (lora_linear_dict[share_module_name].lora_dropout[adapter_name])}))
                        self.lora_A.update(nn.ModuleDict({adapter_name: (lora_linear_dict[share_module_name].lora_A[adapter_name])}))
                        self.lora_B.update(nn.ModuleDict({adapter_name: (lora_linear_dict[share_module_name].lora_B[adapter_name])}))
                        self.scaling[adapter_name] = lora_linear_dict[share_module_name].scaling[adapter_name]
                elif number_adapter == layer_num - 1:
                    self.r[adapter_name] = r
                    self.lora_alpha[adapter_name] = lora_alpha
                    if lora_dropout > 0.0:
                        lora_dropout_layer = nn.Dropout(p=lora_dropout)
                    ...
                    for start, end in middle_share_schedule:
                        if start <= number_key < end:
                            if 0 <= number_key - start < 1:
                                self.r[adapter_name] = r
                                self.lora_alpha[adapter_name] = lora_alpha
                                if lora_dropout > 0.0:
                                    lora_dropout_layer = nn.Dropout(p=lora_dropout)
                                ...
                                share_module_name = re.sub(r"\d+", str(int(start)), current_key, count=1)
                                self.lora_dropout.update(nn.ModuleDict({adapter_name: (lora_linear_dict[share_module_name].lora_dropout[adapter_name])}))
                                self.lora_A.update(nn.ModuleDict({adapter_name: (lora_linear_dict[share_module_name].lora_A[adapter_name])}))
                                self.lora_B.update(nn.ModuleDict({adapter_name: (lora_linear_dict[share_module_name].lora_B[adapter_name])}))
                                self.scaling[adapter_name] = lora_linear_dict[share_module_name].scaling[adapter_name]
...
class MSPLoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def _check_new_adapter_config(self, config: MSPLoraConfig) -> None:
        """
    ...
    def _create_and_replace(
        ...
        **optionnal_kwargs,
        ...
        kwargs["bias"] = bias
        kwargs["current_key"] = optionnal_kwargs.get("current_key", None)
        kwargs["layer_total_number"] = optionnal_kwargs.pop("layer_total_number", -1)
        kwargs["lora_linear_dict"] = optionnal_kwargs.pop("lora_linear_dict", {})
    ...
    def _create_new_module(lora_config, adapter_name, target, **kwargs):
        ...
        lora_linear_dict = kwargs.get("lora_linear_dict", {})
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            lora_linear_dict[kwargs.get("current_key")] = new_module
...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.current_key = kwargs.pop("current_key", None)
        self.middle_share_part_num = 3
        self.middle_share_schedule = []
        self.layer_total_number = kwargs.pop("layer_total_number", -1)
        self.lora_linear_dict = kwargs.pop("lora_linear_dict", {})
        
        if len(r) > 2 and not self.middle_share_schedule:
            for i in range(0, self.middle_share_part_num):
                if not i:
                    start = 0
                end = start + float(self.layer_total_number) / self.middle_share_part_num
                self.middle_share_schedule.append((start, end))
                start = end
        ...
        for i in range(0,len(r)):
            adapter_name_msp = adapter_name + f"_{i}"
            self.update_layer(adapter_name_msp, r[i], lora_alpha[i], lora_dropout, init_lora_weights, self.current_key, len(r), self.middle_share_schedule, self.lora_linear_dict)
    ...
    def get_delta_weight(self, adapter):
        delta_weight = 0
        msp_adapter_names = self.r.keys()
        for adapter_name_i in msp_adapter_names:
            if (
                adapter_name_i in self.lora_A and adapter_name_i in self.lora_B
                and re.match(f"^{adapter}_[0-9]+$", adapter_name_i)
            ):
                delta_weight += (
                    transpose(
                        self.lora_B[adapter_name_i].weight @ self.lora_A[adapter_name_i].weight,
                        self.fan_in_fan_out,
                    )
                    * self.scaling.get(adapter_name_i, 1.0)
                )
        return delta_weight
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            if len(self.r) > 0 and self.merged:
                self.unmerge()
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
        elif len(self.r) > 0 and not self.merged:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
            x = x.to(self.lora_A[next(iter(self.r))].weight.dtype)
            if self.layer_total_number != 12:
                for msp_adapter_name, rank in self.r.items():
                    if rank > 0 and re.match(f"^{self.active_adapter}_[0-9]+$", msp_adapter_name):
                        x = x.to(self.lora_A[msp_adapter_name].weight.device)
                        result += (self.lora_B[msp_adapter_name](self.lora_A[msp_adapter_name](self.lora_dropout[msp_adapter_name](x))) * self.scaling[msp_adapter_name]).to(result.device)
            else:
                for msp_adapter_name, rank in self.r.items():
                    if rank > 0 and re.match(f"^{self.active_adapter}_[0-9]+$", msp_adapter_name):
                       result += (self.lora_B[msp_adapter_name](
                           self.lora_A[msp_adapter_name](self.lora_dropout[msp_adapter_name](x))
                           )
                           * self.scaling[msp_adapter_name]
                       )
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)

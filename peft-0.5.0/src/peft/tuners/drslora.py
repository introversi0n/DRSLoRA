...
@dataclass
class DRSLoraConfig(LoraConfig):
    """
    This is the configuration class to store the configuration of a [`~peft.DRSLora`].
    ...
    tinit: int = field(default=10, metadata={"help": "The steps of initial warmup."})
    tfinal: int = field(default=100, metadata={"help": "The steps of final warmup."})
    ...
    def __post_init__(self):
        self.peft_type = PeftType.DRSLORA


class DRSLoraLayer(LoraLayer):
    def __init__(
        self,
        in_features: int,
        out_features: int,
...
class DRSLoraModel(LoraModel):
    ...
    Args:
        ...
        config ([`DRSLoraConfig`]): The configuration of the DRSLora model.
        adapter_name (`str`): The name of the adapter, defaults to `"default"`.

    Returns:
        `torch.nn.Module`: The DRSLora model.

    Example::

        >>> from transformers import AutoModelForSeq2SeqLM, LoraConfig >>> from peft import DRSLoraModel, DRSLoraConfig
        >>> config = DRSLoraConfig(
                peft_type="DRSLORA", task_type="SEQ_2_SEQ_LM", r=8, lora_alpha=32, target_modules=["q", "v"],
                lora_dropout=0.01,
            )
        >>> model = AutoModelForSeq2SeqLM.from_pretrained("t5-base") >>> model = DRSLoraModel(model, config, "default")

    **Attributes**:
        - **model** ([`transformers.PreTrainedModel`]) -- The model to be adapted.
        - **peft_config** ([`DRSLoraConfig`]): The configuration of the DRSLora model.
    """
    ...
    def __init__(self, model, config, adapter_name):
        ...
        if traininable_mode_counter > 1:
            raise ValueError(
                "DRSLoraModel supports only 1 trainable adapter. "
    ...
    def _create_and_replace(
        ...
        **optionnal_kwargs,
        ...
        if not isinstance(target, DRSLoraLayer):
...
class SVDLinear(nn.Linear, DRSLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        DRSLoraLayer.__init__(self, in_features=in_features, out_features=out_features)
...
if is_bnb_available():
    ...
    class SVDLinear8bitLt(bnb.nn.Linear8bitLt, DRSLoraLayer):
...
if is_bnb_4bit_available():
    ...
    class SVDLinear4bit(bnb.nn.Linear4bit, DRSLoraLayer):
        # Low-rank matrix for SVD-based adaptation
        def __init__(
            self,
            adapter_name,
...
class SVDQuantLinear(torch.nn.Module, DRSLoraLayer):
    def __init__(
        self,
        adapter_name,
        quant_linear_module,
...
class RankAllocator(object):
    """
    The RankAllocator for DRSLoraModel. Paper: https://openreview.net/pdf?id=lq62uWRJjiY

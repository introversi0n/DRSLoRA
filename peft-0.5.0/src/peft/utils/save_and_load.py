...
def get_peft_model_state_dict(model, state_dict=None, adapter_name="default"):
    ...
    if config.peft_type in (PeftType.LORA, PeftType.MSPLORA, PeftType.ADALORA, PeftType.DRSLORA):
...
def set_peft_model_state_dict(model, peft_model_state_dict, adapter_name="default"):
    ...
    if config.peft_type in (PeftType.MSPLORA):
        ...
        # for i in range(0,len(config.r)):
        #     msp_adapter_name = adapter_name + f"_{i}"
        #     for k, v in state_dict.items():
        #         if parameter_prefix in k:
        #             suffix = k.split(parameter_prefix)[1]
        #             if "." in suffix:
        #                 suffix_to_replace = ".".join(suffix.split(".")[1:])
        #                 k = k.replace(suffix_to_replace, f"{msp_adapter_name}.{suffix_to_replace}")
        #             else:
        #                 k = f"{k}.{msp_adapter_name}"
        #             peft_model_state_dict[k] = v
        #         else:
        #             peft_model_state_dict[k] = v
